...
// Type: Spinballs.Core.Actions.ActionSequence
...
{
  [DataContract]
  public class ActionSequence : ActionBase, IActionContainer
  {
    ...
    public override bool Update(GameTime gameTime)
    {
      ...
      {
        if (!this.Actions[index].Finished)
        {
          if (this.Actions[index].Update(gameTime) && index == this.Actions.Count - 1)
          {
            this.Finished = true;
            break;
          }
